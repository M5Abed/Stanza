...
async function run2() { const res = await ai.models.list(); for await (const m of res) { if (m.name.includes('tts') || m.name.includes('flash')) console.log(m.name); } } run2().catch(console.error);
